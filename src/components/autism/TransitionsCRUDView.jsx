...
const API_BASE = 'https://api-digitalsymphony.ngrok.pizza/api/transitions';

export default function TransitionsCRUDDataTable() {
  const columns = [
    { name: "id", label: "ID", editable: false },

    { name: "name", label: "Transition Name", editable: true, required: true },

    // If your CrudDataTable supports selects, add `type: "select", options: [...]`.
    { name: "category", label: "Category", editable: true, required: true, 
      type: "select", options: ["vertical", "horizontal"]
    },
  ];
